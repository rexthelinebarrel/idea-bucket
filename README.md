...
- 📴 **离线语音识别**：内置 sherpa-onnx 本地引擎（默认 22MB 轻量中文模型，可选 188MB 中英双语模型），识别全程不联网、无需任何 Key
...
- **默认离线识别**：内置 sherpa-onnx 本地引擎，识别全程不联网、不需要任何 Key；首次使用下载一次模型即可（轻量版 22MB，国内镜像源直连）。推荐。
- **系统识别**：也可调用手机自带的语音识别（小米/华为等国产机自带中文引擎），免配置。
- **云端 API（可选）**：想要更稳定效果或 AI 讨论时，推荐[硅基流动](https://siliconflow.cn)（国内直连、有免费额度），API 地址填 `https://api.siliconflow.cn/v1`，转写模型以控制台为准（如 `FunAudioLLM/SenseVoiceSmall`），对话模型可填免费的 Qwen 系列。
- 三种转写模式在 App「设置」页随时切换。
